/* moon.js — данные и визуализация страницы «Фаза Луны».
   ...
   и Engine — они уже загружены раньше в index.html. Тексты и заголовки
...
(function (g) {
  ...
  function discSvg(size, illum, waxing) {
    ...
    if (k <= 0.004) {
      ...
      /* Терминатор выгибается ВНУТРЬ диска на серпе (k<0.5) и НАРУЖУ на горбе
         (k>0.5). Внутрь — это дуга, закрученная против обода, наружу — по
         ободу, поэтому на горбе innerSweep совпадает с outerSweep, а на серпе
         противоположен ему. Раньше здесь стояло ровно обратное правило, и диск
         рисовал дополнение к настоящей фазе: при освещённости 5% закрашивалось
         95% круга. Проверено растеризацией: доля светлых пикселей совпадает с
         illum в пределах 0,4 п.п. на 5/25/50/75/95%. */
      var innerSweep = waxing ? (k < 0.5 ? 0 : 1) : (k < 0.5 ? 1 : 0);
