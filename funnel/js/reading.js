/* reading.js — БЕСПЛАТНОЕ персональное чтение по пути к продукту.

   ЧЕМ ОНА БЫЛА И ПОЧЕМУ ИЗМЕНИЛАСЬ. Страница подавалась как то, что
   открывается после оплаты, но проверки оплаты в ней нет и статике её взять
   неоткуда: она открывалась у любого, кто знает адрес. При этом она
   показывает подмножество того, что умеет продукт, — три позиции, разделы,
   неделю, пару, — то есть конкурировала с тем, что продаётся, будучи заведомо
   слабее, и вдобавок написана на двух языках из десяти.

   Поэтому она стала тем, чем фактически была: бесплатным чтением. Оно
   честное и полное, ничего не заблюрено, а в конце — переход в продукт с
   объяснением разницы: чтение это снимок на сегодня, карта движется вместе с
   небом. Деньги открывают продукт, а не эту страницу.
...
(function () {
  ...
  /* Переход в продукт. Разница названа прямо, без нагнетания: это снимок на
     сегодня, а карта движется. Ссылка ведёт в продукт и работает независимо
     от того, оплачено ли — гейт стоит там, а не здесь. */
  var go = document.createElement('section');
  go.className = 'rd__go';
  go.innerHTML = '<h2 class="rd__h"></h2><p class="rd__gotext"></p>' +
    '<a class="cta" id="rdCta"></a>';
  go.querySelector('.rd__h').textContent = U.goTitle;
  go.querySelector('.rd__gotext').textContent = U.goText;
  var cta = go.querySelector('#rdCta');
  cta.textContent = U.goCta;
  cta.href = PRODUCT_URL;
  el('report').appendChild(go);
